<template>
  <div>
    ...
    <select class="form-control" v-model="selectedGenreId" >
      ...
      -->
      <!-- <div class="row mt-3">
        <div class="col-3" v-for="movie in selectm" :key="movie.id">
          <movie-list-item :movie="movie"></movie-list-item>
        </div>
      </div> -->

      <div class="row" v-if="selectedGenreId == 0">
        <movie-list-item
...
export default {
  ...
  methods: {
    ...
    //     this.selectm = this.movies
    //   }
    //   else{
    //     this.selectm = this.movies.filter(e => {
    //       return e.genre_id === this.selectedGenreId
    //     })
    //   }
    // }
  },
  watch:{
    selectedGenreId: {
      handler: function(selectedGenreId){
          this.selectm = this.movies.filter(e =>{
            return e.genre_id === selectedGenreId
          })
      }
    }
  }
